document.addEventListener("DOMContentLoaded", function () {
  ...
  function coreIsEnglish() {
    return window.CCT_LANG === "en";
  }

  function coreText(vi, en) {
    return coreIsEnglish() ? en : vi;
  }
    ...
    return new Promise(function (resolve) {
      ...
      let html =
        '<div class="option-modal-head">' +
          '<div><strong>' + escapeHtml(product.name) + '</strong><div class="option-modal-sub">' + escapeHtml(coreText("Chọn option trước khi thêm vào giỏ", "Choose options before adding to cart")) + '</div></div>' +
      ...
      html +=
        ...
        '<div class="option-modal-actions">' +
          '<button type="button" class="option-cancel-btn">' + escapeHtml(coreText("Huỷ", "Cancel")) + '</button>' +
          '<button type="button" class="option-confirm-btn">' + escapeHtml(coreText("Thêm vào giỏ", "Add to cart")) + '</button>' +
        '</div>';
      ...
      modal.querySelector(".option-confirm-btn").addEventListener("click", function () {
        ...
        for (let i = 0; i < groups.length; i += 1) {
          ...
          if (!checked && groups[i].required !== false) {
            alert(coreText("Vui lòng chọn ", "Please choose ") + (groups[i].name || "option") + ".");
            return;
          }
  ...
  productGrid.addEventListener("click", async function (event) {
    ...
    if (
      ...
      alert(coreText("Món này chỉ còn " + product.stock_qty + " phần.", "Only " + product.stock_qty + " left in stock."));
      return;
    }
  ...
  cartList.addEventListener("click", function (event) {
    ...
    if (delta > 0 && item.stock_qty !== null) {
      ...
      if (totalForProduct >= item.stock_qty) {
        alert(coreText("Món này chỉ còn " + item.stock_qty + " phần.", "Only " + item.stock_qty + " left in stock."));
        return;
      }
    }
