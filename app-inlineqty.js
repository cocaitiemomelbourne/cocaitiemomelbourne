document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderProducts(category) {
    ...
    productGrid.innerHTML = selected
      .map(function (product) {
        ...
        const selectedClass = selectedQty > 0 ? " selected" : "";

        const actionHtml = selectedQty > 0
          ? [
              '<div class="card-qty-control">',
              '<button class="card-qty-btn" type="button" data-card-minus-id="', product.id, '">−</button>',
              '<div class="card-qty-center"><span class="card-selected-label">Đã chọn</span><strong>', selectedQty, '</strong></div>',
              '<button class="card-qty-btn card-qty-plus" type="button" data-add-index="', index, '">+</button>',
              '</div>'
            ].join("")
          : [
  ...
  function updateCart() {
    ...
    if (cart.length === 0) {
      ...
      if (PRODUCTS.length) renderProducts(getActiveCategory());
      return;
    }
