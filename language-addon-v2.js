document.addEventListener("DOMContentLoaded", function () {
  ...
  function applyStaticLanguage() {
    ...
    if (serviceLabels[0]) serviceLabels[0].textContent = "Pick up";
    if (serviceLabels[1]) serviceLabels[1].textContent = "Delivery";
    if (serviceLabels[2]) serviceLabels[2].textContent = "AusPost";
    if (serviceValues[0]) serviceValues[0].textContent = "Springvale";
    if (serviceValues[1]) serviceValues[1].textContent = "Melbourne";
    if (serviceValues[2]) serviceValues[2].textContent = currentLang === "en" ? "from $12" : "từ $12";
    ...
    document.querySelectorAll(".card-selected-label").forEach(function (label) {
      label.textContent = currentLang === "en" ? "Selected" : "Đã chọn";
    });
