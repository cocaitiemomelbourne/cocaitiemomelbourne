document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderProducts(category) {
    ...
    productGrid.innerHTML = selected
      .map(function (product) {
        ...
        const selectedQty = getProductCartQty(product.id);
        const actionHtml = selectedQty > 0
          ? [
              '<div class="card-qty-control" aria-label="Số lượng đã chọn">',
              '<button class="card-qty-btn" type="button" data-card-minus-id="', product.id, '" aria-label="Giảm số lượng">−</button>',
              '<div class="card-qty-center"><span class="card-selected-label">Đã chọn</span><strong>', selectedQty, '</strong></div>',
              '<button class="card-qty-btn" type="button" data-add-index="', index, '" aria-label="Tăng số lượng">+</button>',
              '</div>'
            ].join("")
          : [
        ...
        return [
          '<div class="product-card', selectedQty > 0 ? ' selected' : '', '">',
  ...
  productGrid.addEventListener("click", async function (event) {
    const minusButton = event.target.closest("[data-card-minus-id]");

    if (minusButton) {
      const productId = Number(minusButton.dataset.cardMinusId);
