document.addEventListener("DOMContentLoaded", function () {
  ...
  const I18N = {
    vi: {
      all:"Tất cả", inStock:"Còn ", cartEmpty:"Giỏ hàng đang trống.",
      ...
      pickupRequired:"Vui lòng chọn ngày và giờ pick up.", shippingRequired:"Vui lòng nhập tên người nhận, số điện thoại và địa chỉ nhận hàng.",
      reserveFail:"Không thể giữ tồn kho cho đơn này.", insufficient:"Một món trong giỏ không còn đủ số lượng. Vui lòng tải lại menu và chọn lại.",
      ...
      menuLoadFail:"Không tải được menu. Vui lòng thử lại sau.", noItems:"Hiện chưa có món trong mục này."
    },
    en: {
  ...
  async function loadProducts() {
    try {
      ...
      console.error(error);
      productGrid.innerHTML = '<div style="grid-column:1/-1;padding:20px;background:#fff;border-radius:14px">' + escapeHtml(t("menuLoadFail")) + '</div>';
    }
  }
  ...
  function renderProducts(category) {
    ...
    if (!selected.length) {
      productGrid.innerHTML = '<div style="grid-column:1/-1;padding:20px;background:#fff;border-radius:14px">' + escapeHtml(t("noItems")) + '</div>';
      return;
    }
      ...
      .map(function (product) {
        ...
        const stockText =
          product.stock_qty === null
            ? ""
            : '<div style="font-size:12px;color:#806f61;margin-top:6px">' + escapeHtml(t("inStock")) +
              product.stock_qty +
              "</div>";
  ...
  function updateCart() {
    ...
    if (cart.length === 0) {
      cartList.innerHTML = t("cartEmpty");
      return;
    }
  ...
  function applyStaticLanguage() {
    ...
    const paymentLabel = payOptions ? payOptions.previousElementSibling : null;
